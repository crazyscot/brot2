...
namespace ChunkDivider {
	_CD__BODY(OneChunk) {
		...
		Plot3Chunk * chunk = new Plot3Chunk(s, f, width, height, 0, 0, origin, size, ty);
		...
		for (unsigned i=0; i<nY; i++) {
			...
			for (unsigned j=0; j<nX; j++) {
				Plot3Chunk * chunk = new Plot3Chunk(
						...
						thisRowOrigin, pixSize, ty);
				list_o.push_front(chunk);
				thisRowOrigin += stepX;
			}
			Plot3Chunk * chunk = new Plot3Chunk(
					s, f, lastXsize, SIZE, (width - lastXsize), SIZE*i,
					thisRowOrigin, lastColSize, ty);
			list_o.push_front(chunk);
			origin += stepY;
		}
		// Last row ...
		{
			...
			for (unsigned j=0; j<nX; j++) {
				Plot3Chunk * chunk = new Plot3Chunk(
						...
						thisRowOrigin, lastRowSize, ty);
				list_o.push_front(chunk);
				thisRowOrigin += stepX;
			}
			Plot3Chunk * chunk = new Plot3Chunk(
					s, f, lastXsize, lastYsize, (width - lastXsize),
					(height-lastYsize), thisRowOrigin, lastCornerSize, ty);
			list_o.push_front(chunk);
			origin += stepY;
		}
	}
	...
	void SuperpixelVariable::dividePlot(std::list<Plot3Chunk*>& list_o,
        ...
        Superpixel::dividePlot(list_o, s, f, centre, size, width, height, ty);
    }
