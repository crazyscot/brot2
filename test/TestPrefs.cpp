...
std::unique_ptr<Prefs> TestingKeyfilePrefs::getWorkingCopy() const throw(){
	if (_parent != NULL)
...
TEST(prefs,ThereCanBeOnlyOne) {
	...
	EXPECT_THROW(std::unique_ptr<Prefs> p3 = p.getWorkingCopy(), Exception);
	// Cannot make a child working copy of a working copy:
	EXPECT_THROW(std::unique_ptr<Prefs> p3 = p2->getWorkingCopy(), Exception);
}
...
TEST(prefs,CommitAffectsMasterAndSubsequentChildren) {
	...
	for (int pct=1; pct<100; pct+=42) {
		...
		std::unique_ptr<Prefs> p2 = pm->getWorkingCopy();
	...
	{
		std::unique_ptr<Prefs> p3 = pm->getWorkingCopy();
...
TEST(prefs, UncommittedChangesDoNotPropagate) {
	...
	bool orig_b;
	{
		std::unique_ptr<Prefs> p4 = pm->getWorkingCopy();
		orig_b = p4->get(PREF(ShowControls));
		p4->set(PREF(ShowControls), !orig_b);
		EXPECT_EQ(orig_b, pm->get(PREF(ShowControls)));
	}
	std::unique_ptr<Prefs> p5 = pm->getWorkingCopy();
	EXPECT_EQ(orig_b, pm->get(PREF(ShowControls)));
	delete pm;
}
